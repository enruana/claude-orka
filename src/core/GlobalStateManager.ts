...
function getBuiltinBoardTemplates(): BoardPromptTemplate[] {
  return [
    ...
    {
      id: 'local-default',
      name: 'Local task (no Jira)',
      kind: 'task-init',
      description: 'Internal work that doesn\'t live in Jira — research, docs, TDRs, spikes. Skips Jira sync, still creates a KB entity + optional worktree so the work is traceable.',
      builtin: true,
      requiresWorktree: false,
      body: [
        'You are starting a LOCAL (non-Jira) board task.',
        'Key: {{taskKey}} | Title: {{taskTitle}}',
        'Kind of work: {{taskType}}   ← use this to shape the initial plan',
        'Board: {{boardId}} | Project: {{projectPath}}',
        '',
        'Description from the user:',
        '{{taskDescription}}',
        '',
        'Load the `board-task-init` skill and follow its LOCAL branch.',
        'Do NOT touch Jira — this task is not backed by a ticket.',
        'Create a KB entity matching taskType (research → spike, document → context, design → decision, spike → spike, other → task).',
        'Suggested branch (if a worktree makes sense for this kind of work): {{branchName}}',
        'Worktree parent (only if you decide to create one): {{worktreeParent}}',
        '',
        'When ready, print a compact "ready" summary with the KB entity id you created.',
      ].join('\n'),
    },
    {
      id: 'port-kb-default',
      name: 'Port from KB entity',
      kind: 'task-init',
      description: 'Pre-linked to an existing KB entity — resumes it instead of creating a duplicate. The init skill loads its docs, decisions and history so the new task inherits full context.',
      builtin: true,
      requiresWorktree: false,
      body: [
        'You are RESUMING work on an existing KB entity as a new board task.',
        'Key: {{taskKey}} | Title: {{taskTitle}}',
        'Kind of work: {{taskType}}',
        'Board: {{boardId}} | Project: {{projectPath}}',
        '',
        'Linked KB entity: {{kbEntityId}}   ← do NOT create a new one. Load this and pick up where it left off.',
        '',
        'Description (from the KB entity, editable by user):',
        '{{taskDescription}}',
        '',
        'Load the `board-task-init` skill and follow its LOCAL branch, but Step 1 must:',
        '  1. Read the KB entity {{kbEntityId}} (overview.html, changelog, decisions, linked docs).',
        '  2. Summarize what has already been done and what is still open.',
        '  3. Skip creating a new KB entity — instead, update this one with a "resumed as board task {{taskKey}} on <today>" changelog entry.',
        '',
        'Do NOT touch Jira. Do NOT create a worktree unless the resumed work clearly needs one.',
        'When ready, print a "resumed" summary: entity id, open questions, next step.',
      ].join('\n'),
    },
    {
      id: 'close-default',
      name: 'Wrap up (post-merge cleanup)',
      kind: 'task-close',
      description: 'Assumes the PR is already merged. Verifies merge, removes the worktree via moxikit, closes the KB entity with links to what shipped, leaves a short Jira comment, and moves the ticket to Done if it isn\'t already.',
      builtin: true,
    ...
    {
      id: 'close-keep-worktree',
      name: 'Wrap up + keep worktree',
      kind: 'task-close',
      description: 'Same post-merge cleanup as the default, but leaves the local worktree intact (useful when you\'re about to reuse it for a follow-up task).',
      builtin: true,
